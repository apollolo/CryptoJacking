...
def start_chromium():
	os.system('chromium --headless --no-sandbox --remote-debugging-port=9222 &')

def kill_chromium():
	subprocess.check_call(["pkill", "chromium"])

def init_min_detect():

	min_detect = dict()
	print("int min detect: " + str(min_detect))
	min_detect = dict()
	min_detect['js'] = False
...
def step_2(path, web, min_detect):
	...
	for url in next(os.walk(web_path))[1]:
		...
		print("mindetect after js_found:" + str(min_detect))
		#Detect wasm
		if not wasm_found:
			wasm_found = detect_from_file(url_path, files, patterns['wasm'])
			min_detect['wasm'] = wasm_found
		print("mindetect after wasm_found:" + str(min_detect))
		...
		print("mindetect after html_found:" + str(min_detect))
